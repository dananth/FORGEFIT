...
### Problem
Android WebView blocks programmatic `.click()` calls on hidden `<input type="file">` elements. This caused the Garmin CSV upload button to do nothing on Android.

### Solution — `<label htmlFor>` pattern

The file input is rendered as a visually-hidden element linked to a visible `<label>`:

```jsx
<label htmlFor="garmin-csv-input" style={{ display: "block", cursor: "pointer" }}>
...
Tapping the label is a real user-initiated interaction, which Android WebView allows to open the native file picker.

**Never replace this with `display: none` + `ref.click()`.** That pattern works in desktop browsers but is blocked by Android WebView security.
...
Do not narrow this back to `.csv` extension only — it will silently reject valid files on many Android devices.
...
- **No hidden file inputs** — Android WebView blocks `ref.click()` on `display:none` inputs. Always use the `<label htmlFor>` pattern for file uploads (see Android File Upload section).
